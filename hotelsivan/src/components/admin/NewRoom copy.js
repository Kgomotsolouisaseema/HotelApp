...
import { v4 as uuidv4   } from "uuid"; //Import v4 from the uuid library and use it to randomise characters
...
const NewRoom = () => {
  ...
  const handleUpload = () => {
    const id = uuidv4();
    try {
      const imageRef = ref(storage, `hotelImages/${roomNum}/${imageUpload.name + id()}`); //uuid is used here but i don understand ????
      ...
      <div className="container-add-room">
        <div className="form-add-room">
          ...
          >
            ...
            <option value="Standard Room (1 to 2 People)">
              Double (1 to 2 People)
            </option>
            <option value="Economy Room (1 to 3">Queen (1 to 3)</option>
            <option value="Deluxe Room (1 to 4)">Suite (1 to 4)</option>
          </select>
          <br />
